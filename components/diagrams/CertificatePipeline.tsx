...
export function CertificatePipeline() {
  return (
    <svg
      viewBox="0 0 720 260"
      role="img"
      aria-label="Certificate eligibility pipeline: public program pages are scraped with Python and R into structured requirement rules; student records are queried with SQL into course completions; both feed a business-rule matching layer that produces a validated eligibility list used for outreach."
      className="h-auto w-full"
    >
      <ArrowDefs />

      {/* Top path: public data */}
      <DiagramBox x={10} y={24} w={150} h={54} label="Public pages" sublabel="programs · courses" />
      ...
      <DiagramBox x={588} y={100} w={122} h={60} label="Outreach list" sublabel="100s of students" />
    </svg>
  );
}
